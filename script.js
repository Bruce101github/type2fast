...
const perfomanceAndControls = document.getElementById(
  "perfomance-and-controls",
);
...
const screenBlocker = document.getElementById("screen-block");
...
function filterTargetTexts() {
  ...
  if (state.punctuations === true && state.numbers === true) {
    sampleTexts = allTexts.filter(
      (text) => text.punctuations === true && text.numbers === true,
    );
  } else if (state.punctuations === true && state.numbers === false) {
    sampleTexts = allTexts.filter(
      (text) => text.punctuations === true && text.numbers === false,
    );
  } else if (state.punctuations === false && state.numbers === true) {
    sampleTexts = allTexts.filter(
      (text) => text.punctuations === false && text.numbers === true,
    );
  } else {
    sampleTexts = allTexts.filter(
      (text) => text.punctuations === false && text.numbers === false,
    );
  }
  state.targetText =
    sampleTexts[Math.floor(Math.random() * sampleTexts.length)].text;
}
...
document.addEventListener("keydown", (e) => {
  ...
  if (!state.isCompleted) {
    if (
      ...
      screenBlocker.classList.contains("hide")
    ) {
      state.curCharIndex++;
      state.keyCount++;
      if (e.key === " ") {
      ...
      if (state.mode.name === modes[2].name || modes[3].name) {
        if (e.key === " ") {
          ...
          state.mode.name === modes[2].name &&
          e.key !== state.textChars[state.curCharIndex].key
        ) {
          state.isCompleted = true;
          showResults();
        }
...
function restartSes() {
  ...
  wordCountDisplay.innerText =
    state.mode.name === "words" ? `0 / ${state.mode.selectedOption}` : "0";
  clearInterval(countDownTimerInterval);
  timerDisplay.innerText =
    state.mode.name === "time" ? `${state.mode.selectedOption}s` : "0s";
...
const accuracy = () => {
  if (state.keyCount > 0) {
    state.accuracy = Math.floor(
      ((state.keyCount - state.mistakes) / state.keyCount) * 100,
    );
...
function switchButton() {
  ...
  contentNav = [
    { value: state.punctuations, button: punctuationsBtn },
    { value: state.numbers, button: numbersBtn },
  ];
